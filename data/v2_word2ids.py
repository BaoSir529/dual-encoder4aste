...
def review_embed(word2idx):
    f = open(r'..\embedding\amazon_product_review_corpus.particle_verbs.cbow.w5.d500.txt', 'rb')
    embeddings_dict = {}
    for line in f:
        values = line.split()
        word = values[0].strip().decode('utf-8')
...
def process(path):
    with open(path+r'\train.txt', encoding='utf-8') as f:
        train = f.readlines()
    with open(path+r'\dev.txt', encoding='utf-8') as f:
        dev = f.readlines()
    with open(path+r'\test.txt', encoding='utf-8') as f:
        test = f.readlines()
